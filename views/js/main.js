...
var resizePizzas = function(size) {
  ...
  function Pizza(size) {
      var abc = document.getElementsByClassName("randomPizzaContainer");
      var dx = determineDx(abc[0],size);
       
      var newwidth = (abc[0].offsetWidth + dx) + 'px';
     
       for (var i = 0; i < abc.length; i++) {
      abc[i].style.width = newwidth;
    }
  }
...
function updatePositions() {
  ...
  for (var i = 0; i < items.length; i++) {
    var phase = Math.sin(a + (i % 5));
    items[i].style.left = items[i].basicLeft + 100 * phase + 'px';
  }
//as per given in the review (*the result of the calculation can be stored in a variable outside the for loop for use inside the for loop.*)
